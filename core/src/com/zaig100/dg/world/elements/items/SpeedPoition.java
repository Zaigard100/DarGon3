...
public class SpeedPoition extends Item {

    float power;

    public SpeedPoition(int count,float power) {
        super(count);
        type = ItemType.SPEED_POITION;
        if(power<1){
            power = 1;
        }
        this.power = power;
    ...
    @Override
    public boolean use() {
        if(World.player.speed<power){
            World.player.speed = power;
            return true;
        }
        return false;
    }
