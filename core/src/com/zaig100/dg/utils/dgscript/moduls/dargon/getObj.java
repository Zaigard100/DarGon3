    ...
    public Value execute(Value... args) {
        ...
        if (args.length == 1) {
            ...
            for (int i = 0; i < cond.size(); i++) {
                objVal.set(i, objInValue(cond.get(i)));
            }
            return objVal;
        }
        if (args.length == 2) {
            return objInValue(cond.get((int) args[1].asNum()));
        }
        throw new RuntimeException("Argument count exeption");
    }

    private Value objInValue(Obj obj) {
        ArrayValue arr;
        switch (obj.getType()) {
            case OBJ:
                arr = new ArrayValue(4);
                arr.set(0, new NumberVal(obj.getX()));
                arr.set(1, new NumberVal(obj.getY()));
                arr.set(2, new StringVal(obj.getTag()));
                arr.set(3, new StringVal("OBJ"));
                return arr;
            case ITEM:
                arr = new ArrayValue(6);
                arr.set(0, new NumberVal(obj.getX()));
                arr.set(1, new NumberVal(obj.getY()));
                arr.set(1, new NumberVal(((Item) obj).itemId));
                arr.set(1, NumberVal.fromBoolean(((Item) obj).active));
                arr.set(4, new StringVal(obj.getTag()));
                arr.set(5, new StringVal("ITEM"));
                return arr;
            case ARROW:
            ...
            case BUTTON:
                ...
                arr.set(1, new NumberVal(obj.getY()));
                arr.set(2, new StringVal(obj.getTag()));
                arr.set(3, new StringVal("BUTTON"));
                return arr;
            case SPINNEY:
                arr = new ArrayValue(6);
                arr.set(0, new NumberVal(obj.getX()));
                ...
                arr.set(4, new StringVal(obj.getTag()));
                arr.set(5, new StringVal("SPINNEY"));
                return arr;
            case CROSSBOW:
                arr = new ArrayValue(7);
                arr.set(0, new NumberVal(obj.getX()));
                ...
                arr.set(5, new StringVal(obj.getTag()));
                arr.set(6, new StringVal("CROSSBOW"));
                return arr;
            case TELEPORT:
                arr = new ArrayValue(7);
                arr.set(0, new NumberVal(obj.getX()));
                ...
                arr.set(5, new StringVal(obj.getTag()));
                arr.set(6, new StringVal("TELEPORT"));
                return arr;
            case HIDE_TRAP:
                ...
                arr.set(4, new StringVal("HIDE_TRAP"));
                return arr;
            case FLIMSY_TILE:
                arr = new ArrayValue(6);
                arr.set(0, new NumberVal(obj.getX()));
                ...
                arr.set(4, new StringVal(obj.getTag()));
                arr.set(5, new StringVal("FLIMSY_TILE"));
                return arr;
            case FLAMETHROWER:
                arr = new ArrayValue(6);
                arr.set(0, new NumberVal(obj.getX()));
                ...
                arr.set(2, new NumberVal(((Flamefrower) obj).stage));
                arr.set(3, new NumberVal(((Flamefrower) obj).angle));
                arr.set(2, new StringVal(obj.getTag()));
                arr.set(3, new StringVal("FLAMETHROWER"));
                return arr;
        }
